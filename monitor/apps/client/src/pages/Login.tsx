...
export function Login() {
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); setBusy(true); setError(""); const data = new FormData(e.currentTarget); try { await auth.login(String(data.get("username")), String(data.get("password"))); navigate("/admin", { replace: true }); } catch { setError(t("login.failed")); } finally { setBusy(false); } };
