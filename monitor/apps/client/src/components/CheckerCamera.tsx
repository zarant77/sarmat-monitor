...
export function CheckerCamera({ module, minCellVoltage, maxCellVoltage, onCancel, onConfirm }: {
  ...
  const [latest, setLatest] = useState<CheckerRecognitionResult | null>(null); const [lockedCells, setLockedCells] = useState<number[] | null>(null);
      ...
      if (now - lastAttempt >= ANALYSIS_INTERVAL_MS) {
        ...
        if (video && stage && frame && canvas && video.videoWidth && video.videoHeight) {
          ...
          if (context && sourceWidth > 0 && sourceHeight > 0) {
            ...
            const result = recognizeCheckerImage(context.getImageData(0, 0, ANALYSIS_WIDTH, ANALYSIS_HEIGHT), { min: minCellVoltage, max: maxCellVoltage });
            historyRef.current = [...historyRef.current.slice(-4), result];
            const stability = evaluateStability(historyRef.current);
            setLatest(result); setScannerState(stability.state);
  ...
  }, [ready, lockedCells, error, minCellVoltage, maxCellVoltage]);

  const retry = () => { historyRef.current = []; setLatest(null); setLockedCells(null); setScannerState("red"); setError(""); void videoRef.current?.play(); };
  const visibleCells = lockedCells ?? latest?.cells ?? Array(6).fill(null);
  ...
  return <div className="camera-overlay" role="dialog" aria-modal="true" aria-label={t("camera.title", { module })}>
    ...
    <div className="camera-stage" ref={stageRef}>
      ...
      <div className={`camera-frame scanner-${scannerState}`} ref={frameRef}><span>{t(`camera.state.${scannerState}`)}</span><i/><i/><i/><i/></div>
