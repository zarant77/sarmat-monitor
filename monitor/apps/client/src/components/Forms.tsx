...
export function BatteryForm({ crewId, battery, onClose }: { crewId?: string; battery?: Battery; onClose: () => void }) {
  const [typeId, setTypeId] = useState(battery?.typeId ?? "");
  ...
  const submit = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const d = new FormData(e.currentTarget); mutation.mutate({ ...(battery ? {} : { crewId }), label: d.get("label"), serialNumber: d.get("serialNumber"), typeId: d.get("typeId"), ...(d.has("state") ? { state: d.get("state") } : {}), notes: d.get("notes") }); };
  return <Modal title={battery ? t("forms.editBattery") : t("forms.registerBattery")} eyebrow={t("forms.packIdentity")} onClose={onClose}><form onSubmit={submit} className="form-grid">
    ...
    <label>{t("forms.batteryType")}<select name="typeId" value={typeId} onChange={event => setTypeId(event.target.value)} required><option value="">{t("forms.selectBatteryType")}</option>{battery && !types.data?.some(type => type.id === battery.typeId) && <option value={battery.typeId}>{battery.typeName}</option>}{types.data?.map(type => <option value={type.id} key={type.id}>{type.name}</option>)}</select></label><label>{t("forms.operationalState")}<select name="state" disabled={Boolean(battery?.archivedAt) || battery?.state === "retired"} defaultValue={battery?.state ?? "ready"}><option value="ready">{t("states.ready")}</option><option value="charging">{t("states.charging")}</option><option value="in_use">{t("states.in_use")}</option><option value="storage">{t("states.storage")}</option><option value="service">{t("states.service")}</option>{battery?.state === "retired" && <option value="retired">{t("states.retired")}</option>}</select></label>
